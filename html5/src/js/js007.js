...
btnStyle.addEventListener('click', function() {
  ...
  divStyle.style.backgroundColor = 'rgb(0,0,0)';
});
...
btnImage.addEventListener('click', function() {
  //body比较特殊,可以直接获取到
  document.body.style.backgroundImage = 'url("images/js002.png")';
});

let btnImage1 = document.getElementById('btnImage1');
btnImage1.addEventListener('click', function() {
  //body比较特殊,可以直接获取到
  document.body.style.backgroundImage = 'url("images/js001.png")';
});

/*
js可以控制style的值任何页面元素都有style属性,
该属性可以获取或者设置元素样式通过style,具体样式名称来控制具体的属性值
例如 元素.style.color='#ff0000'来修改元素的字体颜色元素.style.width='100px'来修改元素的宽度
需要注意！！！样式的名称如果带有减号需要转换成驼峰表达式反过来也需要转换
例如. 要设置font-size样式
需要写成 元素.style.fontSize...
同理 元素.style.backgroundImage是设置background-image
样式名称的减号要去掉,然后将减号的字母转大写
反过来就是将大写字母转小写前面补减号
*/
...
setInterval(function() {
  divTimer.innerHTML = divTimer.innerHTML + '计时器添加信息<br>';
}, 1000);
