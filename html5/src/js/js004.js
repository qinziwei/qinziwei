...
//知识点二
//document.getElementById是获取和设置元素文件内容
...
//计算机需要严格区分变量的类型来记录不同数据
//比如记录可以计算的数值,或者可以描述内容的字符串
//数值表现为1,0,1这样的
//字符串要界定在单引号或者双引号中
//例如:'蜘蛛侠',"计算机软件开发"
//由于javascrip语言的特殊性,变量也可以是页面元素
//变量声明格式 let 变量名称=变量值；
//  例如:let name='钢铁侠';
//       let age=48;
//变量名称必须是_或者字母开头,
...
//例如:let inttv=pareInt('123');
//     let v001=pareInt(12.345);
//参数可以是任意类型,但是不保证一定能转换成功
//  let inte=parseInt('灭霸');
let v001 = parseInt('123'); //字符串
let v002 = parseInt('456.78'); //内容是小数的字符串
let v003 = parseInt(456.789); //小数值
...
//  区间随机数公式Matn.random()*(上界-下界+1)+下界
//  注意！！！由于是随即小数,需要将结果转换成整数
