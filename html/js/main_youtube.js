...
(function($) {
  ...
  $.latitude_longitude = [[40.67, -73.94]]; //-- google map latitude and longitude, $.latitude_longitude = [[latitude,longitude]];
  $.map_marker_url = 'img/marker.png'; //-- Map marker image url
  $.map_marker_info =
    '<h6 class="text-darkgrey">Main Office</h6><p class="map-marker-info">Open at 09:00 - 15:00<br>Monday till Saturday</p>';
  $.enable_wordrotator = true; //-- enabling wordrotator on headline text
  $.wordrotator_words = [
    '<h5>爱音乐、爱运动、爱生活</h5>',
    '<h5>html,java的开发，"会找影视"资源的好人</h5>'
  ]; //-- Array of words, for headline text
  ...
  $(window).load(function(e) {
    ...
    var delay = setTimeout(function() {
      ...
      $(
        '.img-logo, nav, .home-section .left-side, .home-section .right-side'
      ).addClass('is-visible');
  ...
  $(document).ready(function(e) {
    ...
    $('.menu-link').on('click', function() {
      ...
      if ($(window).width() < 769) {
        ...
        $('nav.small-device .nav-title').html(
          $('nav.small-device ul')
            .find('.active')
            .data('nav-title')
        );
    ...
    //-- 6.17 activate YouTube video background
      ...
      var vid_elem =
        '<a id="video" class="player" data-property="{videoURL:\'' +
        $.youtube_url +
        "',containment:'.bg-container', showControls:false, autoPlay:true, loop:true, mute:true, startAt:0, opacity:1, addRaster:false, quality:'large'}\"></a>";
    ...
    if ($('.home-slider').is(':visible')) {
      $('.home-slider').owlCarousel({
        ...
        afterAction: function() {
          if ($(window).width() < 768) {
            //-- move pagination
            $('.owl-controls').insertAfter(
              '.owl-item.active .text-container .headline-desc'
            );
    ...
    $('.subscribe-form').validate({
      ...
      submitHandler: function(form) {
        ...
        $(form)
          .find('.form-notif')
          .append('<label class="loading">Please wait</label>')
          .addClass('is-visible');
          ...
          error: function(err) {
            ...
            var show_notif = setTimeout(function() {
              ...
              $(form)
                .find('.form-notif')
                .append(
                  '<label class="notif-error">Could not connect to the registration server. Please try again later.</label>'
                );
              $(form)
                .find('.form-notif')
                .addClass('is-visible');
            ...
            if (data.result == 'success') {
              ...
              var show_notif = setTimeout(function() {
                ...
                $(form)
                  .find('.form-notif')
                  .append(
                    '<label class="notif-success">Thank you for subscribing us.</label>'
                  );
                $(form)
                  .find('.form-notif')
                  .addClass('is-visible');
              ...
              var show_notif = setTimeout(function() {
                ...
                $(form)
                  .find('.form-notif')
                  .append('<label class="notif-error">Error</label>');
                $(form)
                  .find('.form-notif')
                  .addClass('is-visible');
          ...
          if (success) {
            ...
            var show_notif = setTimeout(function() {
              ...
              $(form)
                .find('.form-notif')
                .append(
                  '<label class="notif-success">Thank you for contacting us. We will reply you shortly.</label>'
                );
              $(form)
                .find('.form-notif')
                .addClass('is-visible');
